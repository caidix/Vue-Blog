<template>
  <div class="category">
    ...
    <div class="filter-container">
      <el-button type="primary" icon="el-icon-edit" size="small" @click="handleEdit">新建分类</el-button>
    </div>
    <div>
      <el-table
...
export default {
  ...
  methods: {
    async getList() {
      ...
      let { data } = await api.getCategory();
      console.log(data);
      if (data.code === 0) {
        this.item = data.data;
      }
    ...
    async delCategory(item) {
      this.$confirm("确认删除该分类吗？")
        .then(async _ => {
          const { data } = await api.delCategory({ id: item.id });
          if (data.code === 0) {
            Message({
